...
import cs131.pa5.CarsTunnels.Ambulance;
...
public abstract class Vehicle implements Runnable {
    ...
    public Condition pullOverForAmbulance = lock.newCondition(); //false until ambulance present
    
    public Condition continueDriving = lock.newCondition(); //false until timeout based on speed
     ...
     */
    private boolean pulledOver = false;
    public final void doWhileInTunnel() {
    	lock.lock();
         try {
        	 long timer = ((10 - speed) * 100);
    		 ...
    		 try {
    			 ...
    			 while(timer > 0) {
					...
					pullOverForAmbulance.await(timer, TimeUnit.MILLISECONDS);
     ...
     * long driveStartTime = currenttimemillis
     * ambulanceEnter.await(timer, TimeUnit.MILLISECONDS); //stall for the normal time unless
     * an ambulance enters, in which case execution will continue
     * elapsedTime += currenttimemillis - drivestartime
     * while (tunnel.ambulancePresent) ambulanceExit.await(); //indefinite wait for ambulance(s)
     * 
     */
...
//    }
    ...
    /*
     * method to simulate the behavior of pulling over
     */
    public void pullOverForAmbulance() {
    	
    	//maybe synchronize(lock)????
    	lock.lock();
    	try {
    		pulledOver = true;  	
    		pullOverForAmbulance.signalAll();
    	} finally {
    		lock.unlock();
    	}
    ...
    /*
     * method to simulate the act being signaled to continue driving after being pulled over
     */
    public void continueDriving() {
    	lock.lock();
    	try {
    		pulledOver = false;
    		continueDriving.signalAll();
    	}finally {
    		lock.unlock();
    	}
    }
